...
if( ! class_exists( 'Travel_Directory_CPT' ) ) {
	class Travel_Directory_CPT {
		...
		function load_scripts_css() {
			...
			wp_enqueue_style( 'font-awesome', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.6.3/css/font-awesome.min.css', '', '4.6.3' ); // Source: http://cdnjs.com/libraries/font-awesome
		}
		...
		function save_taxonomy_custom_meta( $term_id ) {
			if ( isset( $_POST['term_meta'] ) && isset( $_POST['term_meta_val'] ) ) {
				$t_id = $term_id;
		...
		public function render_meta_box_attributes() {
			...
			if( defined( 'ICL_LANGUAGE_CODE' ) ) {
				...
				if( $trid ) { 											// if new translated post
					...
					if( ! empty( $post_default ) ) {
						...
						if( ! empty( $category_selected ) )
							$category_selected_id = (int) apply_filters( 'wpml_object_id', $category_selected[0]->term_id, 'travel-dir-category', false, ICL_LANGUAGE_CODE );
				...
				$category_selected = get_the_terms( $post->ID, 'travel-dir-category' );
				if( ! empty( $category_selected ) )
					$category_selected_id = $category_selected[0]->term_id;
				$rating = get_meta_rating( $post->ID );
			}
			...
			echo '<select name="category_id" class="widefat">';
			if ( ! empty( $categories ) ) {
				foreach ( $categories as $category ) {
					$selected = ( is_object( $category_selected[0] ) ) ? selected( $category->term_id, $category_selected_id, false ) : false;
		...
		public function save_meta_box_data( $post_id ) {//var_dump($post_id);
			...
			if( ! empty( $category_id ) )
				wp_set_post_terms( $post_id, array( $category_id ), 'travel-dir-category', false );
